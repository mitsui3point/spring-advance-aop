 ...
 *
 * ** {@link Around} 외에 다른 어드바이스가 존재하는 이유 **
...
public class AspectV6Advice {
    ...
    public void doReturn(JoinPoint joinPoint, Object result) {
        log.info("[return] {} return={}", joinPoint.getSignature(), result);//result 값을 @Around 처럼 조작하지는 못한다.
    }
    @AfterReturning(value = "hello.aop.order.aop.Pointcuts.allOrder()", returning = "result")
    public void doReturn2(JoinPoint joinPoint, Integer result) {// OrderRepository 가 String result 일 경우 해당 Advice 가 작동하는데, Integer result 일 경우 타입이 다르므로 Advice 가 작동하지 않는다.
        log.info("[return2] {} return={}", joinPoint.getSignature(), result);//result 값을 @Around 처럼 조작하지는  못한다.
    }
    ...
    public void doAfterThrowing(JoinPoint joinPoint, Throwable ex) throws Throwable {
        log.info("[ex] {} message={}", joinPoint.getSignature(), ex.getMessage(), ex);//ex 값을 @Around 처럼 조작하지는 못한다.
        throw ex;
    }
